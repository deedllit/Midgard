...
public class WaveSurfaceBuilder8 extends SurfaceBuilder<SurfaceBuilderConfig>  {
	
	private PerlinNoiseGenerator perlinNoise = null ; 
	private SandWaves sandWaves = null ; 
	
	public WaveSurfaceBuilder8(Function<Dynamic<?>, ? extends SurfaceBuilderConfig> configFactory) {
		super(configFactory);
	}
	...
	public void buildSurface(Random random, IChunk chunkIn, Biome biomeIn, int x, int z, int startHeight, double noise,
		...
		if((sum & 1) == 1) {
			height += this.sandWaves.getFlat(i, j) ; 					
		} else {
			height -= this.sandWaves.getFlat(i, j) ; 								
		}
		...
		if((sum & 2) == 2) {
			height += this.sandWaves.getEastWestWaveHeight(i-1, j-1) ; 					
		} else {
			height -= this.sandWaves.getEastWestWaveHeight(i-1, j-1) ; 								
		}
				
		if((sum & 4) == 4) {
			height += this.sandWaves.getEastWestWaveHeight(i-2, j-2) ; 					
		} else {
			height -= this.sandWaves.getEastWestWaveHeight(i-2, j-2) ; 								
		}

		if((sum & 8) == 8) {
			height += this.sandWaves.getNorthSouthWaveHeight(i+1, j+1) ; 					
		} else {
			height -= this.sandWaves.getNorthSouthWaveHeight(i+1, j+1) ; 								
		}

		if((sum & 16) == 1) {
			height += this.sandWaves.getNorthSouthWaveHeight(i+2, j+2) ; 					
		} else {
			height -= this.sandWaves.getNorthSouthWaveHeight(i+2, j+2) ; 								
		}
		...
		*/
		...
		if(height2 > 0 || (localNoiseHole > 0.30f && localNoiseHole < 0.9f)) {
