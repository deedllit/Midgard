...
public class WaveSurfaceBuilder4 extends SurfaceBuilder<SurfaceBuilderConfig>  {
	
	private PerlinNoiseGenerator perlinNoise = null ; 

	
	public WaveSurfaceBuilder4(Function<Dynamic<?>, ? extends SurfaceBuilderConfig> configFactory) {
		super(configFactory);
	}
	...
	public void buildSurface(Random random, IChunk chunkIn, Biome biomeIn, int x, int z, int startHeight, double noise,
		...
		double h1 = this.perlinNoise.noiseAt((double)x * 0.00953125D, (double)z * 0.001553125D, false) * 3 * smooth;
        double h2 = this.perlinNoise.noiseAt((double)x * 0.02953125D + 5, (double)z * 0.004553125D + 5, false) * 5 + 1;
        
		height = Math.max(h1, h2);      
		
		if(height > 0 || (localNoiseHole > 0.3f && localNoiseHole < 0.9f)) {
